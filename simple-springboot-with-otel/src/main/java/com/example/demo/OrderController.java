...
public class OrderController {
    ...
    public ResponseEntity<Map<String, Object>> createOrder(@RequestBody Map<String, Object> request) {
        ...
        try {
            ...
            // Mark the current server span as ERROR so the service map turns red.
            // The auto-instrumentation agent cannot detect this failure because
            // we catch the exception and return a ResponseEntity instead of
            // propagating it — from the agent's perspective the request succeeds.
            Span.current().recordException(e);
            Span.current().setStatus(StatusCode.ERROR, "inventory-service unavailable: " + e.getMessage());
            ...
            Span.current().recordException(e);
            Span.current().setStatus(StatusCode.ERROR, "inventory check failed: " + e.getStatusCode());
        ...
        if (!available) {
            Span.current().setStatus(StatusCode.ERROR, "insufficient inventory for productId: " + request.get("productId"));
        ...
        try {
            ...
            Span.current().recordException(e);
            Span.current().setStatus(StatusCode.ERROR, "payment-service unavailable: " + e.getMessage());
            return ResponseEntity.status(503).body(Map.of(
            ...
            Span.current().recordException(e);
            Span.current().setStatus(StatusCode.ERROR, "payment processing failed: " + e.getStatusCode());
            return ResponseEntity.status(e.getStatusCode()).body(Map.of(
